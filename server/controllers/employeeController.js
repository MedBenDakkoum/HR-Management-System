...
const registerEmployee = [
  ...
  async (req, res) => {
    try {
      ...
      const { name, email, password, role, position, internshipDetails, hireDate } =
        req.body;
      ...
      const employeeData = {
        ...
        hireDate: hireDate 
          ? admin.firestore.Timestamp.fromDate(new Date(hireDate))
      ...
      res.status(201).json({
        ...
        data: {
          employee: {
            id: employee.id,
            _id: employee.id, // For backward compatibility
            name: employee.name,
...
const updateQrCode = [
  ...
  async (req, res) => {
    try {
      ...
      logger.info("QR code updated successfully", { employeeId: employee.id });
...
const updateEmployee = [
  ...
  async (req, res) => {
    try {
      ...
      const { name, email, role, position, internshipDetails, hireDate } = req.body;
      const employeeId = req.params.id;
      ...
      if (hireDate) updateData.hireDate = admin.firestore.Timestamp.fromDate(new Date(hireDate));
...
const requestFaceUpdate = [
  ...
  async (req, res) => {
    try {
      ...
      // Import sendEmailAndNotify if available
      try {
        ...
        logger.warn("Failed to send face update notification email", {
